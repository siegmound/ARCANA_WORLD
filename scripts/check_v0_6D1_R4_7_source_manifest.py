...
p=ROOT/'SOURCE_AUTHORITY_MANIFEST_v0_6D1_R4_7.json'
...
out={'stage':'v0.6D1-R4.7','status':'PASS_R47_SOURCE_MANIFEST' if not failed else 'BLOCKED_R47_SOURCE_MANIFEST','checks_passed':len(d.get('files',[]))-len(failed),'checks_total':len(d.get('files',[])),'failed':failed}
