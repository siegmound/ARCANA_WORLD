...
from arcana_worldsim.scientific_engines.serialization import file_sha256
...
def _run_phase(*, root:Path, phase:str, transitions:int, effects:np.ndarray, freq:np.ndarray,
    ...
    manifest=render_r37c_r1_phase_ini(
    ...
    cmd=('export OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1; '
         f'cd {shlex.quote(wdir)} && conda run -n {shlex.quote(conda_env)} nemo2.4.2 Nemo2_ARCANA_R37C_R1.ini')
...
def _one_chain(*, N:int,rep:int,axis:int,selection_variance:float,base_seed:int,loci:int,outroot:Path,conda_env:str,protocol:R37CSelectionProtocol) -> dict:
    ...
    efficacy=selection_efficacy_metrics(effects,sel_freq,ctl_freq)
    ...
    summary={
        'schema':'ARCANA_R37C_R1_NEMO_SELECTION_CHAIN_V1','stage':R37C_R1_STAGE,'chain_id':chain_id,
        ...
        'selection_efficacy':efficacy,
        'phase_boundary_semantics':'QFREQ_FREQUENCY_STATE_CHAIN__LD_AND_GENOTYPE_PHASE_STATE_NOT_PRESERVED',
        'selection_semantics':'GAUSSIAN_PATCH_LOCAL_OPTIMA__ABSOLUTE_FITNESS__COMPOSITE_BREED_SELECTION_DISPERSE',
        'canonical_write_allowed':False,'automatic_calibration_allowed':False,'scalar_K_eff_authorized':False,
    }
    (root/'R3_7C_R1_SELECTION_CHAIN_SUMMARY.json').write_text(json.dumps(summary,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    return summary


def main() -> int:
    ap=argparse.ArgumentParser(description='Run R3.7C-R1 NEMO 2.4.2 repaired matched directional-selection chains through WSL.')
    ...
    print(json.dumps({'stage':R37C_R1_STAGE,'status':'STARTING_NEMO_SELECTION_SUITE','chain_count':len(jobs),'nemo_run_count':len(jobs)*5,'population_sizes':args.population_sizes,'selection_variances':args.selection_variances,'replicates':args.replicates},indent=2))
    records=[]
    with ThreadPoolExecutor(max_workers=args.parallel_chains) as ex:
        ...
        for f in as_completed(fut):
            ...
            try:
                ...
                rec={'stage':R37C_R1_STAGE,'chain_id':f'N{N}_r{r:03d}_a{a}_sv{sv:g}','population_size':N,'replicate':r,'axis':a,'selection_variance':sv,'status':'ENGINE_FAILED_SELECTION_CHAIN','error':repr(e)}
            records.append(rec); print(f"[R3.7C-R1] {rec['chain_id']} {rec['status']}")
    ...
    efficacy_pass=sum(bool(r.get('selection_efficacy',{}).get('selection_efficacy_gate_pass')) for r in records if r.get('status')=='ENGINE_COMPLETED_SELECTION_CHAIN')
    if complete != len(records):
        status='PARTIAL_NEMO_SELECTION_EVIDENCE'
        exit_code=2
    elif efficacy_pass != len(records):
        status='INVALID_SELECTION_ORACLE_SELECTION_EFFICACY_GATE_FAILED'
        exit_code=3
    else:
        status='NEMO_SELECTION_EVIDENCE_COMPLETE_REVIEW_REQUIRED'
        exit_code=0
    summary={'schema':'ARCANA_R37C_R1_NEMO_SELECTION_SUITE_V1','stage':R37C_R1_STAGE,'status':status,'chain_count':len(records),'complete_chain_count':complete,'selection_efficacy_pass_count':efficacy_pass,'nemo_run_count_expected':len(records)*5,'records':records,'selection_efficacy_gate_semantics':'FAIL_CLOSED_NUMERICAL_LIVENESS__NOT_EFFECT_SIZE_CALIBRATION','canonical_write_allowed':False,'production_runtime_binding_authorized':False,'scalar_K_eff_authorized':False}
    (root/'R3_7C_R1_NEMO_SELECTION_SUITE_SUMMARY.json').write_text(json.dumps(summary,indent=2,sort_keys=True)+'\n',encoding='utf-8')
    print(json.dumps({k:summary[k] for k in ('stage','status','chain_count','complete_chain_count','selection_efficacy_pass_count','nemo_run_count_expected')},indent=2))
    return exit_code
