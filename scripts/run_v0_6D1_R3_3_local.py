...
from rebased_natural_control_runtime_v0_6D1_R3_3 import R33Config, run
...
def main():
    ...
    ap.add_argument("--out-dir", type=Path, default=ROOT / "local_runs" / "v0_6D1_R3_3")
    ap.add_argument("--threads", type=int, default=0)
    ap.add_argument("--fission-off", action="store_true")
    ap.add_argument("--coalescence-off", action="store_true")
    args = ap.parse_args()
    if args.threads > 0:
    ...
    cfg = R33Config(end_age_ma=float(args.end_age_ma),
                    persistent_vicariance_fission_enabled=not args.fission_off,
                    persistent_reconnection_coalescence_enabled=not args.coalescence_off)
    tag = f"210_to_{str(args.end_age_ma).replace('.', 'p')}Ma"
    if args.fission_off: tag += "_FISSION_OFF"
    if args.coalescence_off: tag += "_COALESCENCE_OFF"
    t0 = time.time()
    ...
    print(json.dumps({
        "status": "PASS_LOCAL_R3_3_RUN_COMPLETED",
        "tag": tag,
