...
manifest=root/'SOURCE_AUTHORITY_MANIFEST_v0_6D1_R4_31.json'
...
out={'stage':'v0.6D1-R4.31','status':'PASS_R431_SOURCE_MANIFEST' if not failed else 'BLOCKED_R431_SOURCE_MANIFEST','checks_passed':len((d.get('entries') or []))-len(failed),'checks_total':len(d.get('entries') or []),'failed':failed}
