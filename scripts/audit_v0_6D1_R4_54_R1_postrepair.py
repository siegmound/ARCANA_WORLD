...
root = Path.cwd()
a = json.loads(
    (root / "outputs/v0_6D1_R4_54/R4_54_INTEGRATED_AUDIT.json").read_text()
)
s = json.loads(
    (root / "outputs/v0_6D1_R4_54_SEAL/R4_54_FINAL_SEAL_AUDIT.json").read_text()
)

checks = {
    "r454_complete": str(a.get("status", "")).startswith("PASS_R454_"),
    "r454_sealed": s.get("verdict") == "SEALED",
    "five_engine_results_present":
        len(a.get("engine_dry_run_results", {})) == 5,
...
out = {
    ...
    "status":
        "PASS_R454_R1_ARTIFACT_HASH_PROPERTY_REPAIR_AND_R454_RESEAL_VERIFIED"
        if ok else "BLOCKED_R454_R1_POSTREPAIR_VERIFICATION",
    "checks": checks,
    "checks_passed": sum(checks.values()),
    "checks_total": len(checks),
    "r454_status": a.get("status"),
    "seal_status": s.get("status"),
    "seal_verdict": s.get("verdict"),
    "next_action": a.get("next_action"),
}
p = root / "outputs/v0_6D1_R4_54_R1/R4_54_R1_POSTREPAIR_RESEAL_AUDIT.json"
