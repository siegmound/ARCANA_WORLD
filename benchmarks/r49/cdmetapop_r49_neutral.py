...
contract=Path(sys.argv[1]); expansion_path=Path(sys.argv[2]); out=Path(sys.argv[3]); work=Path(sys.argv[4]); repo=Path(sys.argv[5])
...
try:
    c=json.loads(contract.read_text(encoding='utf-8-sig')); exp=json.loads(expansion_path.read_text(encoding='utf-8-sig')); d=c['engine_input']['drivers']; runtime=int(c['engine_input']['representative_runtime']['steps'])
    if exp.get('job_id')!=c['frozen_parent_job']['job_id']: raise RuntimeError('R4.9 expansion/job contract identity mismatch')
    if float(exp['neutral_end_support_ratio'])!=1.0: raise RuntimeError('R4.9 neutral control must use end support ratio 1.0')
    if exp.get('comparison_target_used') is not False: raise RuntimeError('comparison target leakage forbidden')
    specs=exp.get('additional_replicates') or []
    if len(specs)!=16: raise RuntimeError('R4.9 neutral adapter requires exactly 16 additional replicates')
    ...
    for spec in specs:
        ...
        prow=dict(prows[0]); prow['implement_disease']='N'; popdst=inp/'popvars'/'PopVars_R49N.csv'; write_csv(popdst,[prow],pfields)
        runsrc=inp/'RunVars.csv'; rrows=read_csv(runsrc); rfields=list(rrows[0].keys()); rr=dict(rrows[0]); climate_raw=str(rr.get('cdclimgentime') or '0'); climate_knots=[x.strip() for x in climate_raw.split('|') if x.strip()!='']
        if len(climate_knots)<2: raise RuntimeError('CDMetaPOP neutral control requires >=2 existing CDClimate knots')
        patch=inp/'patchvars'/'PatchVars.csv'; rows=read_csv(patch); fields=list(rows[0].keys()); kscale=0.70+0.60*habitat; first_k_series=None; first_n0=None
        for row in rows:
            try: base=float(str(row.get('K') or '300').split('|')[0])
            except Exception: base=300.0
            k0=max(20,int(round(base*kscale))); kvals=[k0 for _ in climate_knots]; row['K']='|'.join(str(x) for x in kvals); n0=max(10,int(round(k0*0.5))); row['N0']=str(n0)
        ...
        rr['mcruns']='1'; rr['runtime']=str(runtime); rr['output_years']='1'; rr['Popvars']='popvars/PopVars_R49N.csv'; rundst=inp/'RunVars_R49N.csv'; write_csv(rundst,[rr],rfields)
        before={p.relative_to(rw).as_posix() for p in rw.rglob('*') if p.is_file()}; runner="""import random,runpy,sys,numpy as np\nseed=int(sys.argv[1]); src=sys.argv[2]; inp=sys.argv[3]; rv=sys.argv[4]; tag=sys.argv[5]\nrandom.seed(seed); np.random.seed(seed); sys.argv=[src,inp,rv,tag]; runpy.run_path(src,run_name='__main__')\n"""
        proc=subprocess.run([sys.executable,'-c',runner,str(seed),str(src),str(inp),rundst.name,f'R49N_{ri:03d}'],cwd=str(repo/'src'),text=True,capture_output=True,check=False,timeout=600)
        after={p.relative_to(rw).as_posix() for p in rw.rglob('*') if p.is_file()}; new=sorted(after-before); indfiles=sorted([p for p in rw.rglob('*.csv') if 'ind' in p.name.lower() and p not in (popsrc,popdst)],key=generation_key)
        ...
        metrics={'initial_population':n0obs if n0obs>0 else None,'final_population':n1 if n1>0 else None,'initial_occupied_patches':np0 if np0>0 else None,'final_occupied_patches':np1 if np1>0 else None,'runtime_generations':runtime,'new_output_files':len(new),'individual_output_files':len(indfiles),'source_commit':commit,'configured_habitat_support_start':habitat,'configured_gene_flow_rate':gf,'dynamic_k_end_support_ratio':1.0,'cdclimate_knots':climate_knots,'example_patch_k_series':first_k_series,'example_patch_configured_n0':first_n0,'configured_n0_to_k_start_ratio':(first_n0/first_k_series[0]) if first_k_series and first_k_series[0] else None,'forcing_source':'R49_ADDITIONAL_MATCHED_NEUTRAL_CONTROL'}
    ...
    result={'stage':'v0.6D1-R4.9','job_id':c['frozen_parent_job']['job_id'],'engine':'CDMetaPOP','adapter_status':'PASS' if all(r['status']=='PASS' for r in reps) else 'ENGINE_EXECUTION_FAILURE','diagnostic_arm':'ADDITIONAL_MATCHED_NEUTRAL_FORCING','neutral_end_support_ratio':1.0,'comparison_target_used':False,'source_tree_modified':False,'replicates':reps,'canonical_write':False}
except Exception as exc:
    result={'stage':'v0.6D1-R4.9','engine':'CDMetaPOP','adapter_status':'ADAPTER_FAILURE','diagnostic_arm':'ADDITIONAL_MATCHED_NEUTRAL_FORCING','replicates':[],'error':repr(exc),'traceback':traceback.format_exc()[-12000:],'canonical_write':False}
