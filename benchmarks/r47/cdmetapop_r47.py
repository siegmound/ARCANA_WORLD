...
import csv, json, math, random, re, runpy, shutil, subprocess, sys, traceback
...
try:
    ...
    if prof.get('job_id')!=c['frozen_parent_job']['job_id']: raise RuntimeError('repair profile/job contract identity mismatch')
    if rp.get('comparison_target_used') is not False: raise RuntimeError('comparison target leakage forbidden')
    ...
    for spec in c['engine_input']['replicates']:
        ...
        prow=dict(prows[0]); prow['implement_disease']='N'; popdst=inp/'popvars'/'PopVars_R47.csv'; write_csv(popdst,[prow],pfields)
        ...
        knot_ratios=linear_knots(end_ratio,len(climate_knots))
        ...
        for row in rows:
            ...
            k0=max(20,int(round(base*kscale))); kvals=[max(20,int(round(k0*r))) for r in knot_ratios]
            row['K']='|'.join(str(x) for x in kvals)
            row['N0']=str(max(10,int(round(k0*0.5))))
            if first_k_series is None: first_k_series=kvals
        ...
        rr['mcruns']='1'; rr['runtime']=str(runtime); rr['output_years']='1'; rr['Popvars']='popvars/PopVars_R47.csv'; rundst=inp/'RunVars_R47.csv'; write_csv(rundst,[rr],rfields)
        ...
        proc=subprocess.run([sys.executable,'-c',runner,str(seed),str(src),str(inp),rundst.name,f'R47_{ri:03d}'],cwd=str(repo/'src'),text=True,capture_output=True,check=False,timeout=600)
    ...
    result={'stage':'v0.6D1-R4.7','job_id':c['frozen_parent_job']['job_id'],'engine':'CDMetaPOP','adapter_status':'PASS' if all(r['status']=='PASS' for r in reps) else 'ENGINE_EXECUTION_FAILURE','driver_application':'R47_dynamic_PatchVars.K_via_existing_CDClimate_knots__start_state_N0_once__migration_from_frozen_gene_flow','forcing_source':rp['forcing_source'],'dynamic_end_support_ratio':end_ratio,'comparison_target_used':False,'compatibility_shim':'implement_disease=N benchmark-local only','source_tree_modified':False,'replicates':reps,'canonical_write':False}
except Exception as exc:
    result={'stage':'v0.6D1-R4.7','engine':'CDMetaPOP','adapter_status':'ADAPTER_FAILURE','replicates':[],'error':repr(exc),'traceback':traceback.format_exc()[-12000:],'canonical_write':False}
