...
import csv, json, os, random, re, runpy, shutil, subprocess, sys, traceback
from pathlib import Path
import numpy as np

PIN='3516aa4e124c57e2f9f4c1d9f1a3bca735ed9118'
contract=Path(sys.argv[1]); out=Path(sys.argv[2]); work=Path(sys.argv[3]); repo=Path(sys.argv[4])
...
try:
    c=json.loads(contract.read_text(encoding='utf-8-sig')); d=c['engine_input']['drivers']; runtime=int(c['engine_input']['representative_runtime']['steps'])
    ...
    for spec in c['engine_input']['replicates']:
        ...
        # Isolate first generic scenario and preserve the v3.08 benchmark-local implement_disease compatibility shim.
        popsrc=inp/'popvars'/'PopVars.csv'; prows=read_csv(popsrc); pfields=list(prows[0].keys())
        if 'implement_disease' not in pfields:
            pfields.append('implement_disease')
        prow=dict(prows[0]); prow['implement_disease']='N'; popdst=inp/'popvars'/'PopVars_R43.csv'; write_csv(popdst,[prow],pfields)
        # Parameterize only START-state carrying support and migration probability. ARCANA end state is never injected.
        patch=inp/'patchvars'/'PatchVars.csv'; rows=read_csv(patch); fields=list(rows[0].keys())
        kscale=0.70+0.60*habitat
        for row in rows:
            try: kval=max(20,int(round(float(row.get('K') or 300)*kscale)))
            except Exception: kval=300
            row['K']=str(kval); row['N0']=str(max(10,int(round(kval*0.5))))
        ...
        runsrc=inp/'RunVars.csv'; rrows=read_csv(runsrc); rfields=list(rrows[0].keys()); rr=dict(rrows[0]); rr['mcruns']='1'; rr['runtime']=str(runtime); rr['output_years']='1'; rr['Popvars']='popvars/PopVars_R43.csv'; rundst=inp/'RunVars_R43.csv'; write_csv(rundst,[rr],rfields)
        before={p.relative_to(rw).as_posix() for p in rw.rglob('*') if p.is_file()}
        runner="""import os,random,runpy,sys,numpy as np\nseed=int(sys.argv[1]); src=sys.argv[2]; inp=sys.argv[3]; rv=sys.argv[4]; tag=sys.argv[5]\nrandom.seed(seed); np.random.seed(seed); sys.argv=[src,inp,rv,tag]; runpy.run_path(src,run_name='__main__')\n"""
        proc=subprocess.run([sys.executable,'-c',runner,str(seed),str(src),str(inp),rundst.name,f'R43_{ri:03d}'],cwd=str(repo/'src'),text=True,capture_output=True,check=False,timeout=600)
        ...
        n0,npatch0=count_individual_file(indfiles[0]) if indfiles else (0,0); n1,npatch1=count_individual_file(indfiles[-1]) if indfiles else (0,0)
        metrics={'initial_population':n0 if n0>0 else None,'final_population':n1 if n1>0 else None,'initial_occupied_patches':npatch0 if npatch0>0 else None,'final_occupied_patches':npatch1 if npatch1>0 else None,'runtime_generations':runtime,'new_output_files':len(new),'individual_output_files':len(indfiles),'source_commit':commit,'configured_habitat_support':habitat,'configured_gene_flow_rate':gf}
    ...
    result={'stage':'v0.6D1-R4.3','job_id':c['frozen_parent_job']['job_id'],'engine':'CDMetaPOP','adapter_status':'PASS' if all(r['status']=='PASS' for r in reps) else 'ENGINE_EXECUTION_FAILURE','driver_application':'start_state_K_N0_and_migration_probability_on_isolated_pinned_generic_patch_network','compatibility_shim':'implement_disease=N benchmark-local only','source_tree_modified':False,'replicates':reps,'canonical_write':False}
except Exception as exc:
    result={'stage':'v0.6D1-R4.3','engine':'CDMetaPOP','adapter_status':'ADAPTER_FAILURE','replicates':[],'error':repr(exc),'traceback':traceback.format_exc()[-12000:],'canonical_write':False}
