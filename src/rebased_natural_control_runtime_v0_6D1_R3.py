...
R3_STAGE_ID = "v0.6D1-R3.1"
R3_SCOPE = "REBASED_NATURAL_CONTROL_D3_3A_VARIANCE_ORDER_AND_GENE_FLOW_REPAIR"
...
def _migration_subcycled_r3(pop, root_species, guild, trait, metadata, lat, lon, env, dt, cfg: R3Config):
    ...
    for _ in range(n):
        ...
        out = r2.migration(out, hab, root_species, metadata, lat, lon, env["accessible"], cfg.transport_cadence_years, cfg)
    hab = habitats_r3(root_species, guild, trait, out, metadata, env, cfg)
    return out, hab
...
def run(common, a1, metadata_rows, cfg: R3Config):
    ...
    for _macro in range(nmacro):
        for _ in range(nbio_per_macro):
            ...
            G_pre, _, _ = r21.pair_metrics_root(pop, trait, root_species, metadata, lat, lon, cfg)
            ...
            G, contact, td = r21.pair_metrics_root(pop, trait, root_species, metadata, lat, lon, cfg)
