...
R3_STAGE_ID = "v0.6D1-R3.4"
R3_SCOPE = "REBASED_NATURAL_CONTROL_ADMIXTURE_VARIANCE_HEADROOM_AND_SPECIES_BOUND_GENE_FLOW"
R3_DEEP_BIOLOGICAL_COUPLING_ENABLED = False


@dataclass(frozen=True)
class R34Config(r21.R21Config):
    ...
    # R3.4 calibrated safety headroom.  Mutation supply and nonlinear
    # homeostasis remain unchanged; only the non-binding numerical safety cap
    # is raised after the long-horizon headroom audit.
    ...
    # R3.3 symmetric lifecycle closure.  These are not new calibrations: the
    # reconnection check cadence and persistence horizon are locked to the
    # already-authorized D3.2B vicariance cadence/horizon.
    persistent_reconnection_coalescence_enabled: bool = True
    reconnection_check_interval_years: float = 500_000.0
    reconnection_persistence_min_years: float = 2_000_000.0




def _variance_cfg_d3_3a(cfg: R34Config) -> av.AdditiveVarianceConfig:
...
def _d3_barrier_transport_cfg(cfg: R34Config) -> d3b.DiversificationAdequacyConfig:
...
def barrier_cfg(cfg: R34Config) -> bp.BarrierHistoryConfig:
...
def validate_r3(cfg: R34Config, a1) -> None:
    ...
    if abs(cfg.reconnection_check_interval_years - cfg.deme_fission_check_interval_years) > 1e-9:
        raise ValueError("R3.3 reconnection cadence must equal the governed fission-check cadence")
    if abs(cfg.reconnection_persistence_min_years - cfg.vicariance_persistence_min_years) > 1e-9:
        raise ValueError("R3.3 reconnection persistence must equal the governed vicariance persistence horizon")
    if not r21._is_multiple(cfg.reconnection_check_interval_years, cfg.biology_cadence_years):
        raise ValueError("reconnection_check_interval_years must be a multiple of biology cadence")
...
def habitats_r3(comp_species, guild, trait, pop, metadata, env, cfg: R34Config):
    nd = len(pop)
...
def _migration_subcycled_r3(pop, root_species, guild, trait, metadata, lat, lon, env, dt, cfg: R34Config):
...
def _pair_metrics_barrier_coupled(pop, trait, root_species, current_species, metadata, lat, lon, env, cfg: R34Config):
    """D3.2B permeability geometry with D3 current-species gene-flow identity.

    Root lineage remains the immutable provenance key for inherited metadata,
    but reproductive contact/gene flow is computed only among demes that share
    the same *current* species identity, matching the SEALED D3 dynamic-deme
    semantics.  Parent and daughter species therefore cannot continue genetic
    moment mixing merely because they share a root lineage.
    """
    current_idx, current_ids = _root_index(current_species)
    alias_metadata = {}
    for sid in current_ids:
        roots = {str(root_species[i]) for i, cur in enumerate(current_species) if str(cur) == str(sid)}
        if len(roots) != 1:
            raise RuntimeError(f"current species {sid} maps to multiple root lineages: {sorted(roots)}")
        root_sid = next(iter(roots))
        alias_metadata[str(sid)] = metadata[root_sid]
    permeability = bp.connectivity_permeability(env["land_support"], barrier_cfg(cfg))
    resource_dummy = np.zeros((len(trait), 2), dtype=float)
    G, contact, td = d3b._pair_metrics_extended(
        pop, trait, resource_dummy, current_idx, current_ids, alias_metadata, lat, lon,
...
def _significant_fragments_conn(di: int, pop: np.ndarray, connectivity: np.ndarray,
                                root_species: list[str], cfg: R34Config):
...
def update_vicariance_persistence_r3(component_ids, pop, connectivity, root_species,
                                      elapsed_year, state, cfg: R34Config):
...
def apply_mature_fissions_r3(*, component_ids, root_species, current_species, guild, pop, trait, va, gen,
                              ...
                              elapsed_year, age_ma, cfg: R34Config):
    if not mature:
...
def _reconnection_pair_key(a: str, b: str) -> tuple[str, str]:
    return r21._pair_key(str(a), str(b))


def update_reconnection_persistence_r33(component_ids, current_species, contact, ri, clock,
                                         elapsed_year, state, cfg: R34Config):
    """Track stable secondary contact without inventing a new threshold.

    A pair is considered reconnected only when it is still the same current
    species, its effective exchange is *above* the D3.0C speciation ceiling,
    intrinsic RI is *below* the D3.0C species-level RI threshold, and the
    biological isolation clock has eroded below the D3.0C minimum isolation
    horizon.  Persistence uses the same 2 Myr horizon and 0.5 Myr check cadence
    already authorized for D3.2B vicariance.
    """
    gate = r21.sg.SpeciationGateConfig(
    ...
    prev = {tuple(k) if isinstance(k, (tuple, list)) else k: dict(v) for k, v in state.items()}
    now, mature = {}, {}
    max_gap = cfg.reconnection_check_interval_years * 1.01
    n = len(component_ids)
    for i in range(n):
        for j in range(i + 1, n):
            if str(current_species[i]) != str(current_species[j]):
                continue
            key = _reconnection_pair_key(component_ids[i], component_ids[j])
            exchange = r21.sg.effective_exchange_pressure(float(contact[i, j]), float(ri[i, j]))
            connected = (
                exchange > float(gate.maximum_effective_exchange_pressure)
                and float(ri[i, j]) < float(gate.minimum_intrinsic_RI)
                and float(clock[i, j]) < float(gate.minimum_effective_isolation_generations)
            )
            if not connected:
                continue
            old = prev.get(key)
            if old is not None and float(elapsed_year) - float(old.get("last_seen_elapsed_year", elapsed_year)) <= max_gap:
                delta = max(0.0, float(elapsed_year) - float(old.get("last_seen_elapsed_year", elapsed_year)))
                first = float(old.get("first_seen_elapsed_year", elapsed_year))
                persistence = float(old.get("continuous_persistence_years", 0.0)) + delta
            else:
                first = float(elapsed_year)
                persistence = 0.0
            row = {
                "component_a": str(key[0]), "component_b": str(key[1]),
                "species_id": str(current_species[i]),
                "first_seen_elapsed_year": first,
                "last_seen_elapsed_year": float(elapsed_year),
                "continuous_persistence_years": persistence,
                "effective_exchange_pressure": float(exchange),
                "intrinsic_RI": float(ri[i, j]),
                "isolation_clock_generations": float(clock[i, j]),
            }
            now[key] = row
            if persistence + 1e-9 >= cfg.reconnection_persistence_min_years:
                mature[key] = dict(row)
    return now, mature


def _mature_reconnection_groups(component_ids, current_species, mature):
    """Find transitive coalescence groups with SciPy's tested graph routine."""
    n = len(component_ids)
    if n < 2 or not mature:
        return []
    ...
    for a, b in mature:
        if a not in pos or b not in pos:
            continue
        i, j = pos[a], pos[b]
        if current_species[i] != current_species[j]:
            continue
        rows.extend([i, j]); cols.extend([j, i])
    if not rows:
        return []
    data = np.ones(len(rows), dtype=np.uint8)
    graph = csr_array((data, (rows, cols)), shape=(n, n))
    ...
    for lab in sorted(set(int(x) for x in labels)):
        ...
        if len(idx) < 2:
            continue
        # A graph component may contain isolated nodes with a shared default
        # label only if it has a real edge; ensure at least one mature edge.
        ids = {str(component_ids[i]) for i in idx}
        if any(a in ids and b in ids for a, b in mature):
            groups.append([int(i) for i in idx])
    return groups


def _normalized_va_for_component(va_row, root_sid, metadata, cfg: R34Config):
    m = metadata[str(root_sid)]
    sc = np.asarray([
        max(float(m["thermal_niche_sigma_c"]), 1e-9),
        max(float(m["aridity_niche_sigma"]) / 1.55, 1e-9),
        max(float(cfg.body_mass_scale), 1e-9),
    ])
    return np.asarray(va_row, dtype=float) / (sc * sc)


def _pooled_component_moments(indices, pop, trait, va, gen):
    masses = np.asarray([float(pop[i].sum()) for i in indices], dtype=float)
    total = float(masses.sum())
    if total <= 0:
        raise ValueError("cannot coalesce zero-mass components")
    means = np.asarray([trait[i] for i in indices], dtype=float)
    vars_ = np.asarray([va[i] for i in indices], dtype=float)
    first_before = np.sum(masses[:, None] * means, axis=0)
    second_before = np.sum(masses[:, None] * (vars_ + means * means), axis=0)
    mean = first_before / total
    second = second_before / total
    var = np.maximum(second - mean * mean, 0.0)
    g = float(np.average(np.asarray([gen[i] for i in indices], dtype=float), weights=masses))
    return masses, total, mean, var, g, first_before, second_before


def _coalescence_survivor_index(indices, component_ids):
    # Preserve the oldest/least-derived bookkeeping identity deterministically.
    return min(indices, key=lambda i: (str(component_ids[i]).count("_F"), len(str(component_ids[i])), str(component_ids[i])))


def _merge_pair_states(group_indices, survivor_idx, component_ids, root_species, masses,
                       ri_state, clock_state):
    """Population-weight external pair-state history onto the retained deme ID."""
    group = set(int(i) for i in group_indices)
    sid = str(component_ids[survivor_idx])
    total = float(np.sum(masses))
    old_ri, old_clock = dict(ri_state), dict(clock_state)
    keep_indices = [j for j in range(len(component_ids)) if j not in group]
    new_ri = {k: v for k, v in old_ri.items() if k[0] not in {str(component_ids[i]) for i in group} and k[1] not in {str(component_ids[i]) for i in group}}
    new_clock = {k: v for k, v in old_clock.items() if k[0] not in {str(component_ids[i]) for i in group} and k[1] not in {str(component_ids[i]) for i in group}}
    for j in keep_indices:
        if str(root_species[j]) != str(root_species[survivor_idx]):
            continue
        other = str(component_ids[j])
        rv = []; cv = []
        for local, i in enumerate(group_indices):
            k = r21._pair_key(str(component_ids[i]), other)
            rv.append(float(old_ri.get(k, 0.0)))
            cv.append(float(old_clock.get(k, 0.0)))
        k2 = r21._pair_key(sid, other)
        new_ri[k2] = float(np.average(np.asarray(rv), weights=masses)) if total > 0 else 0.0
        new_clock[k2] = float(np.average(np.asarray(cv), weights=masses)) if total > 0 else 0.0
    return new_ri, new_clock


def apply_mature_coalescences_r33(*, component_ids, root_species, current_species, guild, pop, trait, va, gen,
                                  ri_state, clock_state, mature, reconnection_state,
                                  founder_state, vicariance_state, metadata, elapsed_year, age_ma, cfg: R34Config):
    groups = _mature_reconnection_groups(component_ids, current_species, mature)
    if not groups:
        return (component_ids, root_species, current_species, guild, pop, trait, va, gen,
                ri_state, clock_state, reconnection_state, founder_state, vicariance_state, [])
    # Apply disjoint connected groups in deterministic order.  Groups are based
    # on the pre-merge graph and therefore cannot overlap.
    merged_records = []
    blocked = []
    for group in groups:
        species_set = {str(current_species[i]) for i in group}
        roots_set = {str(root_species[i]) for i in group}
        guild_set = {int(guild[i]) for i in group}
        if len(species_set) != 1 or len(roots_set) != 1 or len(guild_set) != 1:
            continue
        masses, total, mean, pooled_va, pooled_gen, first_before, second_before = _pooled_component_moments(group, pop, trait, va, gen)
        root_sid = str(root_species[group[0]])
        q = _normalized_va_for_component(pooled_va, root_sid, metadata, cfg)
        if float(np.max(q)) > float(cfg.variance_ceiling_normalized) + 1e-12:
            blocked.append({"component_ids": [str(component_ids[i]) for i in group], "max_normalized_va": float(np.max(q))})
            continue
        survivor = _coalescence_survivor_index(group, component_ids)
        absorbed = [i for i in group if i != survivor]
        combined_pop = np.sum(pop[group], axis=0)
        first_after = total * mean
        second_after = total * (pooled_va + mean * mean)
        new_ri, new_clock = _merge_pair_states(group, survivor, component_ids, root_species, masses, ri_state, clock_state)
        merged_records.append({
            "group": list(group), "survivor": int(survivor), "absorbed": absorbed,
            "combined_pop": combined_pop, "mean": mean, "va": pooled_va, "gen": pooled_gen,
            "ri_state": new_ri, "clock_state": new_clock,
            "event": {
                "event": "deme_coalescence", "elapsed_year": float(elapsed_year), "age_ma": float(age_ma),
                "retained_component_id": str(component_ids[survivor]),
                "absorbed_component_ids": [str(component_ids[i]) for i in absorbed],
                "species_id": str(current_species[survivor]), "root_species_id": root_sid,
                "population": total,
                "population_conservation_error": float(total - sum(float(pop[i].sum()) for i in group)),
                ...
                "max_normalized_va_after_pooling": float(np.max(q)),
                "reconnection_minimum_persistence_years": float(cfg.reconnection_persistence_min_years),
                "semantic_status": "PERSISTENT_SECONDARY_CONTACT_DEME_COALESCENCE_NOT_DESPECIATION",
            }
        })
    ...
    # Rebuild once from all non-overlapping groups.
    absorbed_all = {i for rec in merged_records for i in rec["absorbed"]}
    by_survivor = {rec["survivor"]: rec for rec in merged_records}
    ids=[]; roots=[]; cur=[]; gs=[]; pops=[]; trs=[]; vas=[]; gens=[]
    # Pair states are sequentially updated because groups are disjoint.  Recompute
    # against the pre-rebuild arrays for each group, then purge after rebuild.
    rr, cc = dict(ri_state), dict(clock_state)
    for rec in merged_records:
        rr, cc = _merge_pair_states(rec["group"], rec["survivor"], component_ids, root_species,
                                    np.asarray([float(pop[i].sum()) for i in rec["group"]]), rr, cc)
    touched_ids = set()
    touched_species = set()
    for i in range(len(component_ids)):
        if i in absorbed_all:
            touched_ids.add(str(component_ids[i])); touched_species.add(str(current_species[i])); continue
        if i in by_survivor:
            rec=by_survivor[i]
            ids.append(str(component_ids[i])); roots.append(str(root_species[i])); cur.append(str(current_species[i])); gs.append(int(guild[i]))
            pops.append(rec["combined_pop"]); trs.append(rec["mean"]); vas.append(rec["va"]); gens.append(rec["gen"])
            touched_ids.add(str(component_ids[i])); touched_species.add(str(current_species[i]))
        else:
            ids.append(str(component_ids[i])); roots.append(str(root_species[i])); cur.append(str(current_species[i])); gs.append(int(guild[i]))
            pops.append(pop[i].copy()); trs.append(trait[i].copy()); vas.append(va[i].copy()); gens.append(float(gen[i]))
    rr, cc = r21._purge_pair_state(ids, rr, cc)
    # A merged component starts a fresh vicariance/reconnection episode. Founder
    # persistence involving any touched bookkeeping component must also restart.
    new_reconnection = {k:v for k,v in reconnection_state.items() if k[0] not in touched_ids and k[1] not in touched_ids}
    new_vicariance = {k:v for k,v in vicariance_state.items() if str(k) not in touched_ids}
    new_founder = {}
    for k,v in founder_state.items():
        comps=set(str(x) for x in v.get("component_ids", []))
        if comps & touched_ids:
            continue
        new_founder[k]=v
    events=[rec["event"] for rec in merged_records]
    return (ids, roots, cur, np.asarray(gs,np.uint8), np.stack(pops), np.stack(trs), np.stack(vas), np.asarray(gens),
            rr, cc, new_reconnection, new_founder, new_vicariance, events)


def run(common, a1, metadata_rows, cfg: R34Config):
    validate_r3(cfg, a1)
    ...
    snapshots.append({"age_ma": cfg.start_age_ma, "elapsed_myr": 0.0,
                      ...
                      "extinction_events_cumulative": 0, "fission_events_cumulative": 0, "coalescence_events_cumulative": 0,
    ...
    for _macro in range(nmacro):
        for _ in range(nbio_per_macro):
            ...
            G_pre, _, _ = _pair_metrics_barrier_coupled(pop, trait, root_species, current_species, metadata, lat, lon, env, cfg)
            ...
            current_idx, _current_ids = _root_index(current_species)
            trait, va, gene_flow_closure = av.gene_flow_moment_mix(
                selected, va, totals, G_pre, ri_before, current_idx, _variance_cfg_d3_3a(cfg)
            )
            ...
            G, contact, td = _pair_metrics_barrier_coupled(pop, trait, root_species, current_species, metadata, lat, lon, env, cfg)
            ...
            if cfg.persistent_reconnection_coalescence_enabled and r21._is_multiple(elapsed, cfg.reconnection_check_interval_years):
                reconnection_state, mature_reconnections = update_reconnection_persistence_r33(
                ...
                if mature_reconnections:
                    (component_ids, root_species, current_species, guild, pop, trait, va, gen,
                     ri_state, clock_state, reconnection_state, founder_state, vicariance_state,
                     coalescences) = apply_mature_coalescences_r33(
    ...
    return {
        ...
        "founder_state": founder_state, "vicariance_state": vicariance_state, "reconnection_state": reconnection_state, "extinction_state": ext_state,
        ...
        "authority": {
            ...
            "persistent_reconnection_coalescence_enabled": bool(cfg.persistent_reconnection_coalescence_enabled),
            "deme_coalescence_graph": "SCIPY_SPARSE_CSGRAPH_CONNECTED_COMPONENTS",
            "deme_coalescence_gate": "D3_0C_INVERSE_EFFECTIVE_EXCHANGE_PLUS_RI_AND_ISOLATION_CLOCK_WITH_D3_2B_2MYR_PERSISTENCE",
            "deme_coalescence_moments": "EXACT_POPULATION_FIRST_AND_SECOND_MOMENT_POOLING_WITH_HARD_VA_CEILING_GATE",
