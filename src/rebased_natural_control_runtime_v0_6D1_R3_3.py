...
R3_STAGE_ID = "v0.6D1-R3.3"
R3_SCOPE = "REBASED_NATURAL_CONTROL_DEME_RECONNECTION_COALESCENCE_AND_FRAGMENTATION_HOMEOSTASIS"
R3_DEEP_BIOLOGICAL_COUPLING_ENABLED = False


@dataclass(frozen=True)
class R33Config(r21.R21Config):
...
def _variance_cfg_d3_3a(cfg: R33Config) -> av.AdditiveVarianceConfig:
...
def _d3_barrier_transport_cfg(cfg: R33Config) -> d3b.DiversificationAdequacyConfig:
...
def barrier_cfg(cfg: R33Config) -> bp.BarrierHistoryConfig:
...
def validate_r3(cfg: R33Config, a1) -> None:
...
def habitats_r3(comp_species, guild, trait, pop, metadata, env, cfg: R33Config):
    nd = len(pop)
...
def _migration_subcycled_r3(pop, root_species, guild, trait, metadata, lat, lon, env, dt, cfg: R33Config):
...
def _pair_metrics_barrier_coupled(pop, trait, root_species, metadata, lat, lon, env, cfg: R33Config):
...
def _significant_fragments_conn(di: int, pop: np.ndarray, connectivity: np.ndarray,
                                root_species: list[str], cfg: R33Config):
...
def update_vicariance_persistence_r3(component_ids, pop, connectivity, root_species,
                                      elapsed_year, state, cfg: R33Config):
...
def apply_mature_fissions_r3(*, component_ids, root_species, current_species, guild, pop, trait, va, gen,
                              ...
                              elapsed_year, age_ma, cfg: R33Config):
    if not mature:
...
def _normalized_va_for_component(va_row, root_sid, metadata, cfg: R33Config):
...
def apply_mature_coalescences_r33(*, component_ids, root_species, current_species, guild, pop, trait, va, gen,
                                  ...
                                  founder_state, vicariance_state, metadata, elapsed_year, age_ma, cfg: R33Config):
...
def run(common, a1, metadata_rows, cfg: R33Config):
    validate_r3(cfg, a1)
