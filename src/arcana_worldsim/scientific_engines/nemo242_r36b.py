...
class Nemo242R36BAdapter(ScientificEngineAdapter):
    descriptor = NEMO_242
    ...
    def _validate(self, exp: ScientificExperiment) -> None:
        ...
        if np.max(np.abs(np.diag(G))) > 1e-15:
            raise ValueError("ARCANA exchange matrix must have zero diagonal; NEMO self-retention is derived")
    ...
    def _write_bridge_files(self, exp: ScientificExperiment, root: Path) -> None:
        ...
        # NEMO forward dispersal matrices are row-stochastic. ARCANA stores
        # explicit off-diagonal effective exchange; the non-exchanged fraction
        # therefore becomes the diagonal self-retention probability.
        nemo_disp = exchange.copy()
        np.fill_diagonal(nemo_disp, 1.0 - exchange.sum(axis=1))
        if not np.allclose(nemo_disp.sum(axis=1), 1.0, atol=1e-12, rtol=0.0):
            raise ValueError("derived NEMO forward dispersal matrix is not row-stochastic")
        np.savetxt(root / "nemo_forward_dispersal_matrix.tsv", nemo_disp, delimiter="\t", fmt="%.17g")
        bridge = {
            ...
            "mapping": {
                ...
                "exchange_matrix_semantics": "ARCANA_OFF_DIAGONAL_EFFECTIVE_EXCHANGE_EXPLICITLY_SUPPLIED",
                "nemo_forward_dispersal_semantics": "ROW_STOCHASTIC_WITH_SELF_RETENTION_DIAGONAL_DERIVED_AS_1_MINUS_ARCANA_ROW_SUM",
            },
            "authority": {
    ...
    def _render_template(self, exp: ScientificExperiment, root: Path) -> Path:
        if self.spec.template_path is None:
            ...
            marker.write_text(
                "ARCANA prepared governed NEMO 2.4.2 bridge files.\n"
        ...
        replacements = {
            ...
            "{{ARCANA_NEMO_FORWARD_DISPERSAL_MATRIX_FILE}}": str((root / "nemo_forward_dispersal_matrix.tsv").resolve()),
            "{{ARCANA_EXPERIMENT_SHA256}}": exp.semantic_sha256,
        }
        optional = {
            "{{ARCANA_QTL_EFFECTS_FILE}}": root / "qtl" / "qtl_effects.tsv",
            "{{ARCANA_QTL_ALLELE_FREQUENCIES_FILE}}": root / "qtl" / "qtl_patch_allele_frequencies.tsv",
            "{{ARCANA_QTL_ENSEMBLE_MANIFEST}}": root / "qtl" / "qtl_ensemble_manifest.json",
            "{{ARCANA_SCENARIO_FILE}}": root / "scenario" / "scenario.json",
        }
        for key, path in optional.items():
            if path.exists():
                replacements[key] = str(path.resolve())
    ...
    def parse(self, experiment: ScientificExperiment, workdir: str | Path, execution: EngineExecutionRecord) -> ScientificEvidenceBundle:
        ...
        return ScientificEvidenceBundle(
            ...
            unsupported_mappings=(
                "ARCANA moment state does not uniquely determine a NEMO genome/QTL realization; executable references must use a governed ensemble mapping.",
