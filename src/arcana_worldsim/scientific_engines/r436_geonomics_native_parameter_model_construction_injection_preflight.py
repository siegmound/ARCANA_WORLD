...
def _import_params_module(path: Path) -> dict[str, Any]:
    """Strict import for ARCANA-materialized native parameter modules.

    These files MUST be self-contained Python modules. No namespace repair is
    permitted here.
    """
...
def _read_geonomics_generated_schema_probe(path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    """Read ONLY the non-scientific Geonomics-generated schema probe.

    Geonomics 1.4.9's defined-layer template contains ``np.ones(...)`` but the
    generated file does not necessarily include ``import numpy as np``. That is
    an upstream template-generator self-containment defect, not ARCANA
    scientific authority.

    R4.36-R2 therefore executes only this installed-runtime-generated probe in
    a minimal controlled namespace containing ``np``. ARCANA's 12 materialized
    native parameter modules remain subject to the strict self-contained import
    path above.
    """
    txt = path.read_text(encoding="utf-8")
    has_np_reference = "np." in txt
    has_numpy_import = bool(
        re.search(r"^\s*(?:import\s+numpy\s+as\s+np|from\s+numpy\b)", txt, re.M)
    )

    ns: dict[str, Any] = {}
    np_namespace_injected = False
    if has_np_reference and not has_numpy_import:
        ns["np"] = np
        np_namespace_injected = True

    exec(compile(txt, str(path), "exec"), ns)
    params = ns.get("params")
    if not isinstance(params, dict):
        raise TypeError(f"{path} did not expose dict `params`")

    return params, {
        "probe_contains_np_reference": has_np_reference,
        "probe_contains_numpy_import": has_numpy_import,
        "controlled_np_namespace_injected": np_namespace_injected,
        "scope": "NON_SCIENTIFIC_SCHEMA_PROBE_ONLY",
        "arcana_materialized_native_parameter_import_path_unchanged": True,
    }
...
def _make_template(gnx: Any, root: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    params, probe_adapter = _read_geonomics_generated_schema_probe(raw)
    evidence = {
        ...
        "schema_probe_namespace_adapter": probe_adapter,
