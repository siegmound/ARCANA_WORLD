...
def _j21_dynamic_payload_authority(
    ...
    native_records = [
        ...
        if not (
            ...
            and str(r.get("name")) in SIDECAR_NAMES
        )
    ]
    sidecar_records = [
        r for r in records
    ...
    for i, rec in enumerate(native_records):
        lname = (
            f"R437_ENV_{i:03d}"
    ...
    for rec in sidecar_records:
        ...
        sidecar_rows.append({
            "variable_name": rec.get("name"),
            "canonical_unit_semantics": "PHYSICAL_UNIT_PRESERVED_FROM_R3_33",
    ...
    out = {
        ...
        "status":
            ...
            if (
                len(native_records) == 149
                and len(sidecar_records) == 2
                and total_native_states == 1341
                and future_change_targets == 1192
                and all_native_ok
                and all_sidecar_ok
            )
            else BLOCKED,
        ...
        "canonical_physical_unit_sidecar_count": len(sidecar_records),
    ...
    return out, dynamic_by_name, native_records
...
def _add_exact_j21_change_schedule(
    ...
    *,
    ...
    if set(canonical_names) != (set(layers.keys()) - {"R437_CONSTRUCTION_SUPPORT"}):
        raise RuntimeError("R439 dynamic layer names do not match R4.37 bound layer names")
...
def _inspect_compiled_j21_schedule(
    ...
    per_t_exact = all(counts.get(t) == 149 for t in range(8))
    schedule_count = len(schedule)

    return {
        "pass": bool(
            initial_exact == 149
            and schedule_count == 1192
            and compiled_exact == 1192
            and per_t_exact
            and first is not None
            and int(first[0]) == 0
            and len(changer.change_info) == 149
        ),
        ...
        "exact_149_changes_per_timestep": per_t_exact,
...
def _j21_dynamic_model_preflight(
    ...
    return {
        ...
        "native_dynamic_layer_count": 149,
        "anchor_state_count": 9,
        "compiled_exact_change_target_count_per_replicate": 1192,
        "compiled_exact_change_target_count_all_replicates": 1192 * pass_count,
        "physical_sidecars_in_changer": False,
...
def build(root: Path) -> dict[str, Any]:
    ...
    j21_payload, dynamic_by_name, native_records = _j21_dynamic_payload_authority(
        root, initial_manifest
    )
    write(
    ...
    j21_dynamic = (
        _j21_dynamic_model_preflight(root, native, dynamic_by_name)
    ...
    checks = {
        ...
        "j21_149_native_2_sidecars":
            j21_payload.get("native_identity_layer_count") == 149
            and j21_payload.get("canonical_physical_unit_sidecar_count") == 2,
        "j21_exact_1341_native_anchor_states":
            j21_payload.get("native_identity_anchor_state_count") == 1341,
        "j21_exact_1192_future_change_targets":
            j21_payload.get("future_exact_change_target_count") == 1192,
        "j21_all_native_states_0_1":
            j21_payload.get("all_native_states_finite_and_0_1") is True,
        "j21_sidecars_preserved_outside_changer":
            j21_payload.get("sidecars_bound_into_geonomics_layers") is False
            and j21_dynamic.get("physical_sidecars_in_changer") is False,
        ...
        "j21_4768_compiled_exact_targets":
    ...
    out = {
        ...
        "j21_compiled_exact_change_targets_all_replicates":
            j21_dynamic.get("compiled_exact_change_target_count_all_replicates"),
        "j21_physical_sidecar_count":
            j21_payload.get("canonical_physical_unit_sidecar_count"),
        "nonliteral_carrier_dynamics_authority_closed": False,
        "autonomous_population_dynamics_authorized": False,
        "autonomous_movement_authorized": False,
        "autonomous_ageing_interpretation_authorized": False,
...
def final_seal(root: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "j21_149_plus_2":
            p.get("native_identity_layer_count") == 149
            and p.get("canonical_physical_unit_sidecar_count") == 2,
        "j21_1341_anchor_states":
            p.get("native_identity_anchor_state_count") == 1341,
        "j21_1192_change_targets":
            p.get("future_exact_change_target_count") == 1192,
        "j21_4_dynamic_preflights":
            d.get("replicate_pass_count") == 4,
        "j21_4768_exact_compiled_targets":
            d.get("compiled_exact_change_target_count_all_replicates") == 4768,
    ...
    out = {
        ...
        "summary": {
            ...
            "j21_physical_sidecar_count":
                a.get("j21_physical_sidecar_count"),
            "nonliteral_carrier_dynamics_authority_closed": False,
            "autonomous_population_dynamics_authorized": False,
            "autonomous_movement_authorized": False,
            "production_execution_queue_authorized": False,
