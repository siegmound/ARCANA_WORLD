...
## R4.37-R3 closure semantics

Live R4.37-R2 closed J21 completely but proved that the J14/J18 public
`Model.add_individuals()` route is not merely typo-sensitive: the Geonomics
1.4.9 source-Species branch requires a GenomicArchitecture and tskit union,
whereas the R4.36 construction probe is intentionally nongenomic.

R4.37-R3 therefore stops calling the incompatible mutation API.

For each of J14 and J18 and each of the four frozen seeds it:
- constructs the exact R4.36 native model;
- verifies seed and unrun state;
- checks every canonical carrier coordinate against the actual native
  Geonomics Landscape bounds with no shift, jitter, interpolation or rounding;
- introspects the installed Geonomics 1.4.9 runtime source and freezes the
  public-initializer incompatibility;
- performs no Individual mutation at all.

A successful R4.37 seal means the validation question is closed, NOT that the
scientific exact initial state has been installed.

R4.38 remains mandatory for an audited exact-initialization adapter and
