...
if($ResolvedVersion -ne "1.4.9"){
    Write-Host "R4.46 BLOCKED: expected Geonomics 1.4.9, got '$ResolvedVersion'"
    exit 21
}
...
$Cmd="cd '$WslRoot' && PYTHONPATH='$WslRoot/src' '$GeonomicsWslPython' '$WslRoot/scripts/run_v0_6D1_R4_46.py'"
& wsl.exe -e bash -lc $Cmd
if($LASTEXITCODE -ne 0){
    Write-Host "R4.46 BLOCKED. Preserve scientific execution evidence."
    exit $LASTEXITCODE
}

Write-Host "=== R4.46 final fail-closed evidence seal ==="
& $Python ".\scripts\audit_v0_6D1_R4_46_seal.py"
if($LASTEXITCODE -ne 0){ exit $LASTEXITCODE }

Write-Host "PASS_R446_INTEGRATED_AND_FINAL_SEAL_RUN"
