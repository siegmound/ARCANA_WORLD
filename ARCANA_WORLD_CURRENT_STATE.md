...
ACTIVE_SUBPHASE_STATUS: READY_FOR_LOCAL_NUMERIC_DERIVATION
ACTIVE_SUBPHASE_SCOPE: NATIVE_ENVIRONMENTAL_SUPPORT_EXPOSURE_DERIVATION
NEXT_ACTION: RUN_R5_17_B3_DERIVE_ENVIRONMENTAL_SUPPORT_LOCALLY_AND_IMPORT_MANIFEST
```
...
R5.7 SEALED
  ...
  -> R5.17 AUTHORIZED_IN_PROGRESS
       ...
       -> B3  READY_FOR_LOCAL_NUMERIC_DERIVATION
```
...
Design: `R5_17_B3_HUMAN_ENVIRONMENTAL_WATER_SUPPORT_DESIGN.md`

Runner: `R5_17_B3_DERIVE_ENVIRONMENTAL_SUPPORT.py`

The runner is fail-closed and must revalidate all three input SHA256 values before computation. It will:

1. derive duration-normalized R3.18 exposures for aridity, temperature, land support, low forage, browse forage and wetland forage;
2. derive transparent phase2-minus-phase1 exposure deltas;
3. retain R3.19 `current_accessible` solely as an exact 0-ka endpoint boundary;
4. copy R3.20 hazard arrays unchanged with their original 15–11 ka coverage;
5. verify the 50-year R3.20 cadence and integral closure diagnostics;
6. output a local NPZ plus repository-importable JSON manifest.

It will not generate `K`, human calorie supply, freshwater supply, settlement density or civilization outcomes.
...
No external runtime is required for B3.
