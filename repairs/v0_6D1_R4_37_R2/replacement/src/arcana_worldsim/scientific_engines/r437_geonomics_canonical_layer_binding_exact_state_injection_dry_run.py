...
def _install_public_add_individuals_149_compat_shim(source_spp: Any) -> dict[str, Any]:
    """Repair an upstream Geonomics 1.4.9 public-wrapper NameError in-process.

    Geonomics 1.4.9 Model.add_individuals() tests:
        isinstance(source_spp, species)
    but `species` is not defined in geonomics.sim.model. It also contains a
    later typo `isinstnace`, which is not reached when the first isinstance()
    succeeds.

    R4.37-R2 sets ONLY that missing module-global symbol to the actual Species
    class for the disposable mechanical test, then restores the module globals.
    No installed Geonomics file is modified.
    """
    import geonomics.sim.model as gmodel

    sentinel = object()
    previous = getattr(gmodel, "species", sentinel)
    gmodel.species = source_spp.__class__
    return {
        "module": gmodel,
        "sentinel": sentinel,
        "previous": previous,
        "upstream_bug": "UNDEFINED_species_SYMBOL_IN_Model_add_individuals",
        "installed_geonomics_file_modified": False,
    }


def _restore_public_add_individuals_149_compat_shim(state: dict[str, Any]) -> None:
    gmodel = state["module"]
    if state["previous"] is state["sentinel"]:
        if hasattr(gmodel, "species"):
            delattr(gmodel, "species")
    else:
        gmodel.species = state["previous"]


def _mechanical_injection_dry_run_one_model(
    mod: Any,
    ...
    spp = mod.comm[0]
    probe_keys = list(spp.keys())
    if len(probe_keys) != 1:
        return {
            "pass": False,
            "error": "CONSTRUCTION_PROBE_COUNT_NOT_ONE",
            "probe_count": len(probe_keys),
        }

    source_template = _copy.deepcopy(spp)
    source_probe_key = list(source_template.keys())[0]

    burned_before = bool(spp.burned)
    # TEST-ONLY API guard override. This disposable model is explicitly not
    # scientific state and is discarded after mechanical placement validation.
    spp.burned = True
    ...
    compat = _install_public_add_individuals_149_compat_shim(source_template)
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            for bi, branch in enumerate(manifest.get("branches") or []):
                expected = [
                    [float(c["x_grid_col"]), float(c["y_grid_row"])]
                    for c in branch.get("carriers") or []
                ]
                observed = []
                branch_mismatch = 0
                for coord in expected:
                    ...
                    mod.add_individuals(
                        ...
                        coords=coord,
                        recip_spp=spp,
                        source_spp=source_template,
                        individs=[source_probe_key],
                    )
                    new_keys = list(set(spp.keys()) - before)
                    if len(new_keys) != 1:
                        branch_mismatch += 1
                        continue
                    ind = spp[new_keys[0]]
                    got = [float(ind.x), float(ind.y)]
                    observed.append(got)
                    err = max(abs(got[0] - coord[0]), abs(got[1] - coord[1]))
                    max_abs_error = max(max_abs_error, err)
                    if got != coord:
                        branch_mismatch += 1

                mismatch_total += branch_mismatch
                observed_total += len(observed)
                branch_results.append({
                    "branch_index": bi,
                    "expected_carrier_count": len(expected),
                    "observed_added_carrier_count": len(observed),
                    "expected_coordinate_digest": _coords_digest(expected),
                    "observed_coordinate_digest": _coords_digest(observed),
                    "coordinate_mismatch_count": branch_mismatch,
                    "pass": (
                        len(observed) == len(expected)
                        and branch_mismatch == 0
                        and _coords_digest(expected) == _coords_digest(observed)
                    ),
                })
    finally:
        spp.burned = burned_before
        _restore_public_add_individuals_149_compat_shim(compat)

    unrun = mod.t == -1 and mod.burn_t == -1 and mod.it == -1
    all_pass = (
        mismatch_total == 0
        and all(r["pass"] for r in branch_results)
        and observed_total == int(manifest.get("total_carrier_count") or -1)
        and unrun
        and spp.burned == burned_before
    )
    return {
        "pass": all_pass,
        "branch_count": len(branch_results),
        "expected_total_carrier_count": manifest.get("total_carrier_count"),
        "observed_added_carrier_count": observed_total,
        "coordinate_mismatch_count": mismatch_total,
        "max_abs_coordinate_error": max_abs_error,
        "model_unrun_after_dry_run": unrun,
        "construction_probe_removed": False,
        "scientific_exact_state_installed": False,
        "test_only_burn_guard_override_used": True,
        "burned_flag_restored": spp.burned == burned_before,
        "public_model_add_individuals_used": True,
        "geonomics_149_public_wrapper_compat_shim_used": True,
        "installed_geonomics_file_modified": False,
        "private_remove_individuals_called_by_arcana": False,
        "private_add_individuals_called_by_arcana": False,
...
def _exact_state_public_api_dry_run(
    ...
    for base in rows:
        ...
        try:
            ...
            mod = gnx.make_model(parameters=pdict, verbose=False)
            seed_ok = mod.seed == seed
            unrun_before = mod.t == -1 and mod.burn_t == -1 and mod.it == -1
            dry = _mechanical_injection_dry_run_one_model(mod, manifest)
            passed = seed_ok and unrun_before and dry.get("pass") is True
            records.append({
                "replicate_index": rep,
                "frozen_seed": seed,
                "model_seed_match": seed_ok,
                "model_unrun_before_dry_run": unrun_before,
                "dry_run": dry,
                "pass": passed,
            })
            del mod
    ...
    return {
        "status":
            "R437_EXACT_STATE_PUBLIC_API_MECHANICAL_DRY_RUN_VALIDATED"
...
def build(root: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "j14_four_mechanical_dry_runs_pass":
            j14.get("replicate_count") == 4
            and j14.get("replicate_pass_count") == 4,
        "j18_four_mechanical_dry_runs_pass":
        ...
        "geonomics_public_wrapper_compat_only":
            all(
                r.get("dry_run", {}).get(
                    "geonomics_149_public_wrapper_compat_shim_used"
                ) is True
                and r.get("dry_run", {}).get(
                    "installed_geonomics_file_modified"
                ) is False
                for d in (j14, j18)
        ...
        "test_only_burn_guard_override_explicit":
            j14.get("test_only_burn_guard_override_used") is True
            and j18.get("test_only_burn_guard_override_used") is True,
        "no_model_run":
...
def final_seal(root: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "burn_override_not_scientific":
            a.get("test_only_burn_guard_override_is_scientific_evidence") is False,
        "no_model_run":
    ...
    out = {
        ...
        "summary": {
            ...
            "j14_mechanical_dry_run_replicates": j14.get("replicate_pass_count"),
            "j18_mechanical_dry_run_replicates": j18.get("replicate_pass_count"),
            "j14_branch_count": j14.get("canonical_branch_count"),
            "j18_branch_count": j18.get("canonical_branch_count"),
            "j14_carrier_count": j14.get("canonical_carrier_count"),
            "j18_carrier_count": j18.get("canonical_carrier_count"),
