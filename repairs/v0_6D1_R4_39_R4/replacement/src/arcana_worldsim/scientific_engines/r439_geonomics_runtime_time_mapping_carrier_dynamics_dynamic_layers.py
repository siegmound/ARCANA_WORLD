...
def _j21_dynamic_model_preflight(
    ...
    for row in seed_rows:
        ...
        try:
            ...
            compat_state = (
                _install_geonomics_149_ndarray_change_dim_metadata_adapter()
            )
            try:
                mod = gnx.make_model(parameters=pdict, verbose=False)
            finally:
                _restore_geonomics_149_ndarray_change_dim_metadata_adapter(
                    compat_state
                )
            compat_evidence = copy.deepcopy(compat_state["evidence"])
            ...
            compat_ok = bool(
                compat_evidence["call_count"] == 1176
                and compat_evidence["metadata_repair_count"] == 1176
                and compat_evidence["already_native_dim_count"] == 0
                and compat_evidence["rejected_shape_count"] == 0
                and compat_evidence["tuple_structure_validated_count"] == 1176
                and compat_evidence["timestep_labels_validated_count"] == 1176
                and compat_evidence["raster_transpose_performed"] is False
                and compat_evidence["raster_values_modified"] is False
            )
            records.append({
                "replicate_index": rep,
                "frozen_seed": seed,
                ...
                "removed_r437_static_bound_dynamic_sidecars":
                    bound_dynamic_sidecars,
                "ndarray_change_dim_metadata_adapter": compat_evidence,
                "ndarray_change_dim_metadata_adapter_exact": compat_ok,
                "installed_geonomics_file_modified": False,
                "compiled_schedule": compiled,
                "autonomous_population_dynamics_executed": False,
                "autonomous_movement_executed": False,
                "pass": bool(
                    seed_ok and unrun and T_ok and movement_off
                    and compat_ok and compiled["pass"]
                ),
            })
            del mod
    ...
    compat_pass_count = sum(
        bool(r.get("ndarray_change_dim_metadata_adapter_exact"))
        for r in records
    )
    total_metadata_repairs = sum(
        int(r.get("ndarray_change_dim_metadata_adapter", {}).get(
            "metadata_repair_count", 0
        ))
        for r in records
    )
    total_tuple_validations = sum(
        int(r.get("ndarray_change_dim_metadata_adapter", {}).get(
            "tuple_structure_validated_count", 0
        ))
        for r in records
    )
    total_timestep_validations = sum(
        int(r.get("ndarray_change_dim_metadata_adapter", {}).get(
            "timestep_labels_validated_count", 0
        ))
        for r in records
    ...
    return {
        ...
        "ndarray_change_dim_metadata_adapter_pass_count": compat_pass_count,
        "ndarray_change_dim_metadata_repair_count_all_replicates":
            total_metadata_repairs,
        "ndarray_change_tuple_structure_validation_count_all_replicates":
            total_tuple_validations,
        "ndarray_change_timestep_label_validation_count_all_replicates":
            total_timestep_validations,
        "expected_metadata_repair_count_all_replicates": 4704,
        "raster_transpose_performed": False,
...
def build(root: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "j21_ndarray_dim_metadata_adapter_4_of_4_and_4704":
            j21_dynamic.get(
                "ndarray_change_dim_metadata_adapter_pass_count"
            ) == 4
            and j21_dynamic.get(
                "ndarray_change_dim_metadata_repair_count_all_replicates"
            ) == 4704
            ...
            and j21_dynamic.get(
                "raster_values_modified_by_dim_adapter"
            ) is False
    ...
    out = {
        ...
        "j21_compiled_exact_change_targets_all_replicates":
            j21_dynamic.get("compiled_exact_change_target_count_all_replicates"),
        "j21_ndarray_dim_metadata_adapter_pass_count":
            j21_dynamic.get("ndarray_change_dim_metadata_adapter_pass_count"),
        "j21_ndarray_dim_metadata_repair_count_all_replicates":
            j21_dynamic.get(
                "ndarray_change_dim_metadata_repair_count_all_replicates"
            ),
        "j21_ndarray_tuple_structure_validation_count_all_replicates":
            j21_dynamic.get(
                "ndarray_change_tuple_structure_validation_count_all_replicates"
            ),
        "j21_ndarray_timestep_label_validation_count_all_replicates":
            j21_dynamic.get(
                "ndarray_change_timestep_label_validation_count_all_replicates"
            ),
...
def final_seal(root: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "j21_ndarray_dim_metadata_adapter_exact":
            d.get("ndarray_change_dim_metadata_adapter_pass_count") == 4
            and d.get(
                "ndarray_change_dim_metadata_repair_count_all_replicates"
            ) == 4704
            and d.get(
