...
def test_compiled_schedule_inspects_change_function_defaults():
    ...
    assert "expected_schedule_count" in s
    assert "expected_layer_count" in s
    assert "counts.get(t) == expected_layer_count" in s
...
def test_dynamic_model_preflight_restores_dim_adapter():
    ...
    assert 'probe_compat["metadata_repair_count"] == 8' in s
    assert 'direct_evidence["metadata_repair_count"] == 1176' in s


def test_r4_lyr_series_tuple_contract_is_validated():
    s=inspect.getsource(m._install_geonomics_149_ndarray_change_dim_metadata_adapter)
...
def test_r5_bounded_preflight_avoids_full_dynamic_changer_materialization():
    s=inspect.getsource(m._j21_dynamic_model_preflight)
    assert "probe_name = canonical_names[0]" in s
    assert "BOUNDED_NATIVE_CHANGER_SCHEMA_PROBE" in s
    assert '"full_147_layer_landscape_changer_materialized": False' in s
    assert '"fourfold_redundant_changer_compilation_performed": False' in s
    assert "len(direct_records) == 1176" in s
