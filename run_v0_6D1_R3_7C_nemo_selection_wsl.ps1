...
$Out = Join-Path $Root "local_runs\v0_6D1_R3_7C_SELECTION"
$env:PYTHONPATH = Join-Path $Root "src"
$popArgs = @(); foreach ($n in $PopulationSizes) { $popArgs += "$n" }
$selArgs = @(); foreach ($s in $SelectionVariances) { $selArgs += "$s" }
python (Join-Path $Root "scripts\run_nemo_r37c_selection_chain.py") $Out `
  ...
  --conda-env $CondaEnv
if ($LASTEXITCODE -ne 0) { throw "R3.7C NEMO selection suite failed with exit code $LASTEXITCODE" }
