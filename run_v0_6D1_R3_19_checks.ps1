...
$bt = Join-Path $root ".pytest_tmp_r319_checks"
New-Item -ItemType Directory -Force -Path $bt | Out-Null

python -m pytest -q (Join-Path $root "tests\test_r319_phase_aware_transport_closure.py") --basetemp (Join-Path $bt "r319")
...
$out = Join-Path $root "outputs\v0_6D1_R3_19\FORMAL_AUDIT_CANDIDATE_v0_6D1_R3_19.json"
python (Join-Path $root "scripts\formal_audit_v0_6D1_R3_19_candidate.py") --root $root --out $out
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
