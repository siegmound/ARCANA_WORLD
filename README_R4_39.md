...
\n\n## R4.39-R5 bounded native changer preflight\n\nThe R4 live preflight was stopped after about 20 minutes while Geonomics eagerly\nmaterialized the large dynamic LandscapeChanger. R5 preserves complete target\ncoverage without constructing the 147-layer changer.\n\nIt performs one deterministic real native changer integration probe (first\ndynamic layer, all 8 transitions), verifies all four frozen seed bindings, and\npasses all 147x8=1176 canonical targets through governed `_make_lyr_series()`\nusing a real Geonomics Layer.\n\nNo full 147-layer changer, changer execution, or model timestep is performed.\n